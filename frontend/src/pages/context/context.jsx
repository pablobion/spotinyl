...
export default function vinylProvider({ children }) {
    ...
    const handleChangeMusicCurrentVinyl = async (mode) => {
        ...
        try {
            const response = await fetch(`https://api.spotify.com/v1/me/player/${mode}`, requestOptions);

            if (response.status === 204) {
                console.log(`Comando ${mode} executado com sucesso`);
            } else {
                console.error("Erro ao avançar para a próxima música:", response.status);
            }
        } catch (error) {
...
export function useVinylContext() {
    ...
    const { currentVinyl, setCurrentVinyl, currentVinylStatus, handleChangeStatusCurrentVinyl, playing, setPlaying, vinylList, setVinylList, handleChangeMusicCurrentVinyl } = context;
